...
public class CarFleetFilterActivity extends AppCompatActivity implements CarFilterAdapter.onClick {

    private CarFleetFilterActivity activity = this;
    ...
    private void initView() {
        ...
        if (!FleetFragment.groupSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.groupSelectionList){
        ...
        if (!FleetFragment.passengerSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.passengerSelectionList){
        ...
        if (!FleetFragment.doorSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.doorSelectionList){
        ...
        if (!FleetFragment.suitcaseSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.suitcaseSelectionList){
        ...
        if (!FleetFragment.transmissionSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.transmissionSelectionList){
        ...
        if (!FleetFragment.fuilSelectionList.isEmpty()){
            for (CarFilterModel model : FleetFragment.fuilSelectionList){
    ...
    public void saveGroupData(){
        ...
        FleetFragment.groupValue = "";
        if (groupSelectionList.isEmpty()){
            FleetFragment.groupValue = "";
        }else {
            for (int i=0; i<groupSelectionList.size(); i++){
                CarFilterModel model = groupSelectionList.get(i);
                if (TextUtils.isEmpty(FleetFragment.groupValue)){
                    FleetFragment.groupValue = model.getId();
                }else {
                    if (!FleetFragment.groupValue.contains(model.getId())){
                        FleetFragment.groupValue = FleetFragment.groupValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.groupSelectionList.clear();
        for (CarFilterModel model : groupSelectionList){
            FleetFragment.groupSelectionList.add(model);
        }
    ...
    public void savePassengerData(){
        ...
        if (passengerSelectionList.isEmpty()){
            ...
            for (int i=0; i<passengerSelectionList.size(); i++){
                ...
                if (TextUtils.isEmpty(FleetFragment.passengerValue)){
                    FleetFragment.passengerValue = model.getId();
                }else {
                    if (!FleetFragment.passengerValue.contains(model.getId())){
                        FleetFragment.passengerValue = FleetFragment.passengerValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.passengerSelectionList.clear();
        for (CarFilterModel model : passengerSelectionList){
            FleetFragment.passengerSelectionList.add(model);
        }
    ...
    public void saveDoorData(){
        ...
        FleetFragment.doorValue = "";
        if (doorSelectionList.isEmpty()){
            FleetFragment.doorValue = "";
        }else {
            for (int i=0; i<doorSelectionList.size(); i++){
                CarFilterModel model = doorSelectionList.get(i);
                if (TextUtils.isEmpty(FleetFragment.doorValue)){
                    FleetFragment.doorValue = model.getId();
                }else {
                    if (!FleetFragment.doorValue.contains(model.getId())){
                        FleetFragment.doorValue = FleetFragment.doorValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.doorSelectionList.clear();
        for (CarFilterModel model : doorSelectionList){
            FleetFragment.doorSelectionList.add(model);
        }
    ...
    public void saveSuitcaseData(){
        ...
        if (suitcaseSelectionList.isEmpty()){
            ...
            for (int i=0; i<suitcaseSelectionList.size(); i++){
                ...
                if (TextUtils.isEmpty(FleetFragment.suitcaseValue)){
                    FleetFragment.suitcaseValue = model.getId();
                }else {
                    if (!FleetFragment.suitcaseValue.contains(model.getId())){
                        FleetFragment.suitcaseValue = FleetFragment.suitcaseValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.suitcaseSelectionList.clear();
        for (CarFilterModel model : suitcaseSelectionList){
            FleetFragment.suitcaseSelectionList.add(model);
        }
    ...
    public void saveTransmissionData(){
        ...
        FleetFragment.transmissionValue = "";
        if (transmissionSelectionList.isEmpty()){
            ...
            for (int i=0; i<transmissionSelectionList.size(); i++){
                ...
                if (TextUtils.isEmpty(FleetFragment.transmissionValue)){
                    FleetFragment.transmissionValue = model.getId();
                }else {
                    if (!FleetFragment.transmissionValue.contains(model.getId())){
                        FleetFragment.transmissionValue = FleetFragment.transmissionValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.transmissionSelectionList.clear();
        for (CarFilterModel model : transmissionSelectionList){
            FleetFragment.transmissionSelectionList.add(model);
        }
    ...
    public void saveFuilData(){
        ...
        FleetFragment.fuilValue = "";
        if (fuilSelectionList.isEmpty()){
            FleetFragment.fuilValue = "";
        }else {
            for (int i=0; i<fuilSelectionList.size(); i++){
                CarFilterModel model = fuilSelectionList.get(i);
                if (TextUtils.isEmpty(FleetFragment.fuilValue)){
                    FleetFragment.fuilValue = model.getId();
                }else {
                    if (!FleetFragment.fuilValue.contains(model.getId())){
                        FleetFragment.fuilValue = FleetFragment.fuilValue + "," + model.getId();
                    }
                }
            }
        }

        FleetFragment.fuilSelectionList.clear();
        for (CarFilterModel model : fuilSelectionList){
            FleetFragment.fuilSelectionList.add(model);
        }
    ...
    private void onClick() {
        ...
        binding.txtApply.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                FleetFragment.isApplyFilter = true;
                saveGroupData();
                savePassengerData();
                saveDoorData();
                saveSuitcaseData();
    ...
    private void setGroupData(JsonList jsonList){
        if (jsonList.size()!=0){
            ...
            for (int i=0;i<jsonList.size();i++){
                ...
                groupList.add(model);
//                if (model.getId().equals(FleetFragment.groupValue)){
...
//                if (model.getId().equals(FleetFragment.passengerValue)){
//                    selectionValue = model.getName();
//                }
            }
    ...
    private void setDoorData(JsonList jsonList){
        if (jsonList.size()!=0){
            ...
            for (int i=0;i<jsonList.size();i++){
                ...
                model.setStatus("");
                doorsList.add(model);
//                if (model.getId().equals(FleetFragment.doorValue)){
//                    selectionValue = model.getName();
//                }
            }
...
//                if (model.getId().equals(FleetFragment.suitcaseValue)){
//                    selectionValue = model.getName();
//                }
            }
...
//                if (model.getId().equals(FleetFragment.transmissionValue)){
//                    selectionValue = model.getName();
//                }
            }
    ...
    private void setFuelData(JsonList jsonList){
        if (jsonList.size()!=0){
            ...
            for (int i=0;i<jsonList.size();i++){
                ...
                model.setStatus(json.getString(P.status));
                fuelList.add(model);
//                if (model.getId().equals(FleetFragment.fuilValue)){
//                    selectionValue = model.getName();
//                }
            }
